...
for gs in gesture_sets:
    for seq in gs.sequences:
        sample = np.concatenate(list(map(lambda x: x.frame, seq.frames))) # a sample is the concatenation of all the frames in a single seq
        samples.append(sample)
        labels.append(int(gs.label))

# X is an array of size num_frames*len(frame) x num(samples)
